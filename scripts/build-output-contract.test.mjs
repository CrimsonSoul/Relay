...
test('production build emits bounded non-empty application chunks and a separate PDF worker', async () => {
  const outputRoot = await mkdtemp(path.join(tmpdir(), 'relay-build-output-contract-'));

  try {
    const result = spawnSync(process.execPath, [electronViteCli, 'build', '--outDir', outputRoot], {
      cwd: repositoryRoot,
      encoding: 'utf8',
    ...
    assert.equal(importsChunk(dndVendor, reactVendor), true, 'dnd-vendor must import react-vendor');
    assert.equal(
