...
export const UserAvatar: React.FC<{ name: string; color: string; border: string }> = ({ name, color, border }) => (
  <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: color, color: '#fff', border: `1px solid ${border}`, fontSize: '12px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
    {name.charAt(0).toUpperCase()}
  </div>
);

interface PersonInfoProps { label: string; value: string; contactLookup: Map<string, Contact> }
...
export const PersonInfo: React.FC<PersonInfoProps> = ({ label, value, contactLookup }) => {
  if (!value || value === '-' || value === '0') return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', opacity: 0.3 }}>
      <span style={{ color: 'var(--color-text-tertiary)', fontWeight: 600, fontSize: '12px', width: '80px', letterSpacing: '0.05em' }}>{label}</span>
  ...
  const allNames = parts.map(p => { const c = contactLookup.get(p.toLowerCase()); return c ? c.name : p; }).join('; ');
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '15px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden', minWidth: 0 }}>
        ...
        <Tooltip content={allNames}><span className="text-truncate" style={{ color: 'var(--color-text-primary)', fontWeight: 600, display: 'block', maxWidth: '100%' }}>{displayName}{parts.length > 1 && <span style={{ opacity: 0.5, marginLeft: '6px', fontSize: '12px' }}>+{parts.length - 1}</span>}</span></Tooltip>
      </div>
    </div>
  );
};
