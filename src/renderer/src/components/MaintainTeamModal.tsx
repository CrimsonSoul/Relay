...
export const MaintainTeamModal: React.FC<MaintainTeamModalProps> = ({ isOpen, onClose, teamName, initialRows, contacts, onSave }) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setRows((items) => { const oldIndex = items.findIndex((r) => r.id === active.id); const newIndex = items.findIndex((r) => r.id === over.id); return arrayMove(items, oldIndex, newIndex); });
    }
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Edit Team: ${teamName}`} width="900px">
      <div style={{ display: "flex", flexDirection: "column", height: "65vh" }}>
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "10px 16px", display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <TactileButton variant="ghost" style={{ width: "100%", marginTop: "12px", border: "1px dashed rgba(255,255,255,0.1)", opacity: 0.8, height: "48px" }} onClick={handleAddRow}>+ Add Row</TactileButton>
        </div>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px", marginTop: "24px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
