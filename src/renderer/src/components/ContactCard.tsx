...
export const ContactCard = memo(({ name, email, title, phone, avatarColor, action, style, sourceLabel, groups = [], selected, onContextMenu, onRowClick }: ContactRowProps) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => { for (const entry of entries) setIsWide(entry.contentRect.width > 900); });
  ...
  return (
    <div ref={containerRef} onContextMenu={(e) => onContextMenu?.(e, { name, email, title, phone, groups })} onClick={onRowClick} style={{ width: '100%', height: '100%', ...style, display: 'flex', alignItems: 'center' }}>
      <div style={{ width: '100%', height: 'calc(100% - 8px)', display: 'flex', alignItems: 'center', paddingLeft: `${cardPadding + 4}px`, paddingRight: `${cardPadding}px`, background: selected ? 'rgba(59, 130, 246, 0.08)' : 'rgba(255, 255, 255, 0.02)', border: selected ? '1px solid rgba(59, 130, 246, 0.2)' : '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '12px', gap: `${cardPadding}px`, transition: 'all 0.2s ease', cursor: 'default', position: 'relative', overflow: 'hidden' }} className="contact-card-hover">
        <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '4px', background: color, opacity: 0.6 }} />
        <Avatar name={name} email={email} />
        <div style={{ display: 'flex', flexDirection: isWide ? 'row' : 'column', alignItems: isWide ? 'center' : 'stretch', flex: 1, minWidth: 0, gap: isWide ? '32px' : '4px', justifyContent: isWide ? 'space-between' : 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, gap: '4px', flex: isWide ? 1 : 'unset' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
              <Tooltip content={displayName}><span style={{ fontSize: isWide ? '24px' : '22px', fontWeight: 800, color: 'var(--color-text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '-0.02em', display: 'block' }}>{displayName}</span></Tooltip>
          ...
          <div style={{ display: 'flex', alignItems: 'center', minWidth: 0, gap: isWide ? '24px' : '12px', justifyContent: isWide ? 'flex-end' : 'flex-start' }}>
            {formattedPhone ? <Tooltip content={formattedPhone}><span style={{ fontSize: isWide ? '22px' : '20px', color: '#60A5FA', fontWeight: 700, letterSpacing: '0.05em', whiteSpace: 'nowrap', minWidth: isWide ? 'auto' : '180px', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}>{formattedPhone}</span></Tooltip> : !isWide && <div style={{ minWidth: '160px', height: '18px', flexShrink: 0 }} />}
            {groups.length > 0 && (
              <Tooltip content={groups.join(', ')}><div style={{ display: 'flex', gap: '6px', alignItems: 'center', cursor: 'help', flexWrap: 'nowrap', overflow: 'hidden' }}>
                {groups.slice(0, isWide ? 3 : 1).map(g => <GroupPill key={g} group={g} />)}
                {groups.length > (isWide ? 3 : 1) && <span style={{ fontSize: '12px', color: 'var(--color-text-tertiary)', fontWeight: 700, background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '6px', flexShrink: 0 }}>+{groups.length - (isWide ? 3 : 1)}</span>}
              </div></Tooltip>
            )}
          </div>
        </div>
