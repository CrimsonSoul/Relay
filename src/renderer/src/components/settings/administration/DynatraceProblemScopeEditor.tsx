...
type DynatraceProblemScopeEditorProps = {
  profiles: RelayAdministrationSettingSummary | undefined;
...
function describeProblemScope(
  ...
  if (method === 'profiles') {
    ...
    return { label: 'Alerting profiles', description };
  }
...
export function DynatraceProblemScopeEditor({
  profiles,
  execute,
  onFeedback,
}: Readonly<DynatraceProblemScopeEditorProps>) {
  ...
  const replaceProblemScope = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        onFeedback('Stored Dynatrace problem scope updated.');
      }
    } finally {
