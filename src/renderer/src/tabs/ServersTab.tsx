...
const VirtualRow = memo(({ index, style, data }: ListChildComponentProps<VirtualRowData>) => {
  ...
  return <ServerCard style={style} server={server} contactLookup={contactLookup} onContextMenu={onContextMenu} hasNotes={!!noteEntry?.note} tags={noteEntry?.tags} onNotesClick={() => onNotesClick(server)} />;
});
...
export const ServersTab: React.FC<ServersTabProps> = ({ servers, contacts }) => {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '20px 24px 24px 24px', background: 'var(--color-bg-app)', overflow: 'hidden' }}>
      ...
      {h.contextMenu && <ContextMenu x={h.contextMenu.x} y={h.contextMenu.y} onClose={() => h.setContextMenu(null)} items={[
        { label: getServerNote(h.contextMenu.server.name) ? 'Edit Notes' : 'Add Notes', onClick: () => { setNotesServer(h.contextMenu!.server); h.setContextMenu(null); }, icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg> },
        { label: 'Edit Server', onClick: h.handleEdit, icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> },
        { label: 'Delete Server', onClick: h.handleDelete, danger: true, icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> }
      ]} />}
