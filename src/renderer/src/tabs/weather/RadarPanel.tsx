...
const ExternalViewButton: React.FC<{ location: Location }> = ({ location }) => {
  const radarUrl = `https://www.rainviewer.com/map.html?loc=${location.latitude},${location.longitude},8&oFa=0&oC=1&oU=0&oCS=1&oF=0&oAP=1&c=3&o=90&lm=1&layer=radar&sm=1&sn=1`;
  
  return (
    <Tooltip content="Open radar in browser for a larger view" position="top">
      <button 
        onClick={() => globalThis.window.api?.openExternal?.(radarUrl)}
        style={{ 
          position: "absolute", 
          bottom: "8px", 
          left: "8px", 
...
export const RadarPanel: React.FC<RadarPanelProps> = ({ location }) => {
  const radarUrl = location 
    ? `https://www.rainviewer.com/map.html?loc=${location.latitude},${location.longitude},8&oFa=0&oC=1&oU=0&oCS=1&oF=0&oAP=1&c=3&o=90&lm=1&layer=radar&sm=1&sn=1`
    : "";
