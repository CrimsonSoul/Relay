...
export async function getGroups(rootDir: string): Promise<BridgeGroup[]> {
  ...
  try {
    ...
    try {
      ...
      loggers.fileManager.error('[GroupOperations] JSON parse error:', { error: parseError, path });
      return [];
    }
  } catch (e) {
...
export async function saveGroup(
  ...
  try {
    ...
    await modifyJsonWithLock<BridgeGroup[]>(
      path,
      (groups) => {
        const now = Date.now();
...
export async function importGroupsFromCsv(rootDir: string): Promise<boolean> {
  try {
    ...
    const groupColIndex = headers.findIndex(
      (h) =>
        String(h).toLowerCase() === 'group_name' ||
        String(h).toLowerCase() === 'group' ||
        String(h).toLowerCase() === 'name',
    );
    const emailColIndex = headers.findIndex(
      (h) =>
        String(h).toLowerCase() === 'email' ||
        String(h).toLowerCase() === 'member' ||
        String(h).toLowerCase() === 'contact',
    );

    if (groupColIndex === -1 || emailColIndex === -1) {
      loggers.fileManager.error(
        '[GroupOperations] CSV missing required columns (group_name/name, email)',
      );
      return false;
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const email = String(row[emailColIndex] ?? '')
        .trim()
        .toLowerCase();
