...
Relay preserves the complete custom expression, including its internal `or` and `and` clauses, and
places it inside one owned `filter (...)` stage after deduplication and before Relay's fixed
projection, sort, and limit. Expressions may reference `event.status_transition`. Do not include
`fetch`, a leading `filter` pipe, other pipeline stages, comments, or control characters.
...
compatible with clients that know only the alerting-profile list. A profile update from such a
client selects profile mode and clears custom DQL rather than combining the two mechanisms.
